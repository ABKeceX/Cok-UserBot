...
@register(outgoing=True, pattern="^Fltrs|.filters$")
...
CMD_HELP.update({
    "filter":
    "`Fltrs` | `.filters`\
    \nUsage: Melihat filter Cok-UserBot yang aktif di obrolan.\
    \n\n`Fltr` | `.filter <keyword> <balasan> atau balas ke pesan ketik .filter <keyword>\
