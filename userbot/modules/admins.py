...
async def spider(spdr):
    ...
    await spdr.edit(
        ...
        f"**User ID:** `{user.id}`\n"
        f"**Mampus Cok:** `Cmut by {ALIVE_NAME}`"
    )
    if mute(spdr.chat_id, user.id) is False:
        return await spdr.edit("`Error! Pengguna Sudah Dibisukan.`")
    try:
...
@register(outgoing=True, pattern=r"^\.uncmut(?: |$)(.*)")
async def unmoot(unmot):
    # Admin or creator check
    chat = await unmot.get_chat()
    admin = chat.admin_rights
...
CMD_HELP.update(
    {
        ...
        "\n↳ : Membisukan seseorang untuk `.cmut` untuk mengmute mereka."
        "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.cmut` <username/balas ke pesan> <alasan (optional)>"
        "\n↳ : Membisukan ke semua grup yang kamu punya sebagai admin."
        "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.uncmut` <username/reply>"
        "\n↳ : Reply ke pesan seseorang untuk `.uncmut` untuk melepas mereka dari cmut."
        "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.zombies`"
